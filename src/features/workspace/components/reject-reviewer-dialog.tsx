...
const REJECT_OPTIONS: Array<{ target: RejectTarget; labelKey: string; destructive?: boolean }> = [
  ...
  { target: REJECT_TARGET_BOTH, labelKey: 'actions.rejectBothReviewers', destructive: true },
]
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle>{t('dialogs.reject.chooseReviewers.title')}</DialogTitle>
              <DialogDescription className="mt-1">
                {t('dialogs.reject.chooseReviewers.description', { taskName })}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
